THREE.NormalControls = function (camera, domElement) {
    ...
    this.camera = camera;
    this.element = (domElement !== undefined) ? domElement : document;
...
THREE.NormalControls.prototype.updateTouchPos = function (event) {
    if (event.touches[0].clientX === undefined || event.touches[0].clientY === undefined)
        return;

    if (event.touches.length == 1) {
        event.preventDefault();

        this.prevMousePos[0] = event.touches[0].clientX;
        this.prevMousePos[1] = event.touches[0].clientY;
    }
}
THREE.NormalControls.prototype.onTouchMove = function (event) {
    var mouseMotion = [event.touches[0].clientX - this.prevMousePos[0], event.touches[0].clientY - this.prevMousePos[1]];
    switch (event.touches.length) {
        ...
        case 2:
            ...
            var zoomTouch = p1.distanceTo(p2);
            if (zoomTouch > this.zoomFlag) {
                rolled = 30;
                this.zoomFlag = p1.distanceTo(p2);
            } else {
                rolled = -30;
                this.zoomFlag = p1.distanceTo(p2);
            }
            ...
            //    //this.translate(new THREE.Vector3(event.touches[2].clientX - this.prevMousePos[0] * scaleFactor, event.touches[2].clientY - this.prevMousePos[1] * scaleFactor, 0));
...
THREE.NormalControls.prototype.onMouseMove = function (event) {
    ...
    if (this.mouseButton[0] && this.keyboard[82]) {
        ...
        this.camera.up.applyMatrix4(rotation);
    } else if (this.mouseButton[2] || (this.mouseButton[0] && this.keyboard[16])) { //우클릭 이벤트
        var scaleFactor = 0.01 * Math.pow(this.target.clone().sub(this.camera.position).lengthSq(), 1 / 3);
        scaleFactor = Math.max(scaleFactor, 0.01);
        this.translate(new THREE.Vector3(mouseMotion[0] * scaleFactor, mouseMotion[1] * scaleFactor, 0));
    } else if (this.mouseButton[0]) { // 좌클릭 이벤트
        var scaleFactor = -.005;
        this.rotateAroundTarget(mouseMotion[0] * scaleFactor, mouseMotion[1] * scaleFactor);
    } else
        return;
