Thingiview = function (containerId) {
    ...
    this.initScene = function () {
        ...
        spotLight = new THREE.DirectionalLight(0xffffff, .7, 0);
    ...
    onRendererTouchMove = function (event) {
        if (event.touches.length == 1) {
            event.preventDefault();
            //targetXRotation 이 실제 뷰에 영향을 주는듯 350줄
    ...
    this.centerCamera = function () {
        if (geometry) {
            ...
            //수정
            //spotLight.position.x = geometry.center_x;
            //spotLight.position.y = geometry.center_y;
            //spotLight.position.z = geometry.center_z;

            //pointLight.position.x = geometry.center_x;
            //pointLight.position.y = geometry.center_y;
            //pointLight.position.z = geometry.center_z;

            //spotLight.position.x = geometry.min_y * 2;
            //spotLight.position.y = geometry.min_y * 2;
            //spotLight.position.z = geometry.max_z * 2;
        ...
        if (scene && geometry) {
            if (objectMaterial == 'wireframe') {
                ...
                if (isWebGl) {
                    ...
                    //수정
                    material = new THREE.MeshLambertMaterial({ color: objectColor, shading: THREE.SmoothShading });
