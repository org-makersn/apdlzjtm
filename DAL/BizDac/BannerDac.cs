...
//                IList<BannerT> banners = (IList<BannerT>)session.CreateSQLQuery(searchQuery).AddEntity(typeof(BannerT)).List<BannerT>();
//                session.Flush();
...
//        }
        ...
        public IList<BannerT> GetBannerInFront()
        {
            using (ISession session = NHibernateHelper.OpenSession())
            {
                IList<BannerT> list = session.QueryOver<BannerT>().Where(w => w.PublishYn == "y" && w.Type == 1).List<BannerT>();
                return list;
            }
        }
    }
