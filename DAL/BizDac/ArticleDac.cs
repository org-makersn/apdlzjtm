...
namespace Makersn.BizDac
{
    public class ArticleDac : BizDacHelper
    {
        ...
        //                                SUM(Total)
        //                                for fortype IN ([public],[saved],[1001],[1002],[1003],[1004],[1005],[1006],[download],[printing])
        //                            ) as pvt", start, end, (int)(MakersnEnumTypes.OrderState.구매완료));
        ...
        //	                            from ORDER_REQ with(nolock)
        //	                            where ORDER_PATH = 'M' and ORDER_STATUS = '{2}' and REG_DT >= '{0}' and REG_DT < '{1}' 
        //	                            group by month(REG_DT)
        //                            ) as tb
        //                            PIVOT
        //                            (
        ...
        public IList<ArticleT> GetMemberArticleByNo(string memberNo, string gubun, int visitorNo)
        {
            ...
            using (ISession session = NHibernateHelper.OpenSession())
            {
                IList<object[]> results = session.CreateSQLQuery(query)
                                        .SetParameter("memberNo", memberNo)
                                        .SetParameter("visitorNo", visitorNo).List<object[]>();
                session.Flush();
        ...
        public ArticleDetailT GetArticleDetailByArticleNo(int articleNo, int visitorNo)
        {
            string query = @"select 
	                                        ...
	                                        (SELECT COUNT(0) FROM LIKES WHERE ARTICLE_NO = A.[NO] AND MEMBER_NO = :visitorNo) AS IS_LIKES, '0' as UPLOAD_CNT, '0' as DRAFT_CNT,
                                            A.VIDEO_URL 
            ...
            using (ISession session = NHibernateHelper.OpenSession())
            {
                ArticleDetailT detailT = session.CreateSQLQuery(query).AddEntity(typeof(ArticleDetailT))
                                        .SetParameter("visitorNo", visitorNo)
                                        .SetParameter("articleNo",articleNo)
                                        .UniqueResult<ArticleDetailT>();
                session.Flush();
            ...
            using (ISession session = NHibernateHelper.OpenSession())
            {
                ...
                string query = @"SELECT TOP 4 A.NO, ISNULL(AF.IMG_NAME, AF.RENAME) AS MAIN_IMAGE, 
                            ...
                            FROM ARTICLE A WITH(NOLOCK) INNER JOIN ARTICLE_FILE AS AF WITH(NOLOCK)
					                            ...
					                            ON A.MEMBER_NO = M.NO 
                                                WHERE (M.DEL_FLAG != 'Y' OR M.DEL_FLAG IS NULL)
                                                AND A.VISIBILITY = 'Y'  AND A.MEMBER_NO = :memberNo AND A.NO != :articleNo ORDER BY A.REG_DT DESC";


                IList<object[]> results = session.CreateSQLQuery(query)
                    .SetParameter("memberNo",a.MemberNo)
                    .SetParameter("articleNo",no)
                    .List<object[]>();
                //session.Flush();
        ...
        public IList<ArticleDetailT> GetListByOption(int memberNo, int codeNo, string gubun, string globleType, int fromPage, int toPage)
        {
            ...
            targetOptQuery = @"SELECT OutQ.* , '0' as COPYRIGHT, '' as VISIBILITY, '' as OPT, '0' as VIEWCNT, '' as TEMP, '' as REG_IP, '' as REG_ID, OutQ.REG_DT, '' as RECOMMEND_YN, OutQ.REG_DT as RECOMMEND_DT, '0' as UPLOAD_CNT, '0' as DRAFT_CNT, '' as MEMBER_PROFILE_PIC FROM 
                            ...
                            TD.TAG,
                            TD.CONTENTS,
                            M.NAME AS MEMBER_NAME,  A.MEMBER_NO, A.CODE_NO, A.VIEWCNT,
        ...
        public int GetTotalCountByOption(int memberNo, int codeNo, string isMain, string gubun, string globalType)
        {
            ...
            if (globalType != "")
            {
                whereQuery += " AND TD.LANG_FLAG = :globalType ";
            }
            else
            {
            ...
            using (ISession session = NHibernateHelper.OpenSession())
            {
                ...
                if (targetCntQuery.Contains(":globalType"))
                {
                    queryObj.SetParameter("globalType", globalType);
                }
                ...
                using (ITransaction transaction = session.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        if (!string.IsNullOrEmpty(delno))
                        {
                            ...
                            //int cnt = (int)session.CreateSQLQuery(delfileQuery).ExecuteUpdate();
                        }
                        ...
                        //session.CreateSQLQuery(updfileQuery).ExecuteUpdate();
        ...
        public void DeleteArticle(int articleNo)
        {
            using (ISession session = NHibernateHelper.OpenSession())
            {
                using (ITransaction transaction = session.BeginTransaction())
                {
                    ...
                    session.CreateSQLQuery(deletFileQuery)
                        .SetParameter("articleNo",articleNo)
                        .ExecuteUpdate();
                    //}
        ...
        public void CheckTempFile(string temp)
        {
            using (ISession session = NHibernateHelper.OpenSession())
            {
                using (ITransaction transaction = session.BeginTransaction())
                {
                    string changeFileQuery = @"UPDATE ARTICLE_FILE SET FILE_GUBUN='DELETE' WHERE FILE_GUBUN='TEMP' AND TEMP = :temp ";

                    session.CreateSQLQuery(changeFileQuery)
