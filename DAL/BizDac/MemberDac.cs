...
//            if (endDt != "")
//            {
//                query += " AND REG_DT <='" + endDt + " 23:59:59'";
//            }
//            if (name != "")
//            {
...
//            if (endDt != "")
//            {
//                query += " AND DEL_DT <='" + endDt + " 23:59:59'";
//            }
//            if (name != "")
//            {
//                query += " AND NAME LIKE '" + name + "%'";
...
//                            (SELECT Count(0) FROM PRINTER p WHERE p.PRT_MEMBER_NO = m.NO ) AS PrinterCnt ,
//                            p.NAME
//	                        FROM MEMBER m 
...
//            if (endDt != "")
//            {
//                query += " AND m.REG_DT <='" + endDt + " 23:59:59'";
//            }
//            if (name != "")
//            {
...
//        }
        ...
        /// <param name="member"></param>
        /// <returns></returns>
        public int UpdateProfilePic(MemberT member)
        {
            using (ISession session = NHibernateHelper.OpenSession())
            {
                string query = string.Format(@"update MEMBER set PROFILE_PIC = :ProfilePic where NO = :memberNo", member.ProfilePic, member.No);

                int result = (int)session.CreateSQLQuery(query)
                                         .SetParameter("ProfilePic",member.ProfilePic)
                                         .SetParameter("memberNo",member.No)
                                         .ExecuteUpdate();
                session.Flush();
                return result;
            }
        ...
        public ProfileT GetCntList(int memberNo)
        {
            string query = string.Format(@"SELECT
                                ...
                                (SELECT COUNT(0) FROM   LIST WITH(NOLOCK) WHERE MEMBER_NO = :memberNo ) AS ListCnt", memberNo);
            using (ISession session = NHibernateHelper.OpenSession())
            {
...
//                        break;
//                    case "기간선택":
//                        add_where = string.Format(" MAIN.REG_DT >= {0} AND MAIN.REG_DT <= {1} ", "'" + start + "'", "'" + end + " 23:59:59" + "'");
//                        break;
//                    default:
//                        add_where = " 1 = 1 ";
//                        break;
...
//	                            from MEMBER M, PRINTER_MEMBER MAIN with(nolock)
//	                            where M.LEVEL < 50 and M.NO = MAIN.MEMBER_NO and MAIN.REG_DT >= '{0}' and MAIN.REG_DT < '{1}'
//                                group by convert(date, MAIN.REG_DT)
//	                            union all
//	                            select 
//		                            convert(date, MAIN.REG_DT) as Gbn,'uploadedprinter' as fortype, COUNT(1) as Total
//	                            from PRINTER MAIN with(nolock)
//                                where MAIN.REG_DT >= '{0}' and MAIN.REG_DT < '{1}'
//                                group by convert(date, MAIN.REG_DT)
//	                            union all
//	                            select 
//		                            convert(date, MAIN.REG_DT) as Gbn,'order' as fortype, COUNT(1) as Total
//	                            from ORDER_REQ MAIN with(nolock)
//                                where MAIN.ORDER_STATUS = {2} and MAIN.REG_DT >= '{0}' and MAIN.REG_DT < '{1}'
...
//								from ORDER_REQ MAIN, ORDER_DETAIL OD with(nolock)
//                                where MAIN.ORDER_STATUS = {2} and MAIN.NO = OD.ORDER_NO and MAIN.REG_DT >= '{0}' and MAIN.REG_DT < '{1}'
//                                group by convert(date, MAIN.REG_DT), MAIN.ORDER_STATUS
//                            ) as tb
//                            PIVOT
//                            (
...
//        }
        ...
        public void TempUpdateMember(MemberT data)
        {
            using (ISession session = NHibernateHelper.OpenSession())
            {
                MemberT member = session.QueryOver<MemberT>().Where(w => w.No == data.No).Take(1).SingleOrDefault<MemberT>();
