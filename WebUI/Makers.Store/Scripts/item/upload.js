...
Upload.ItemUpload = function () {
    ...
    $('#insert_form').attr("action", "/Item/Upload").ajaxForm(options).submit();
}
...
//업로드 된 파일 리스트
Upload.GetItemFiles = function (no) {
...
Upload.ItemDelete = function () {
    if (confirm("삭제 하시겠습니까?") == true) {
        ...
        $('#insert_form').attr("action", "/Item/Delete").ajaxForm(options).submit();
    }
}
...
Upload.UploadCancle = function () {
    if (confirm("페이지를 벗어나면 저장되지 않은 \n내용은 사라집니다.")) {
        ...
        Ajax.AjaxResponseModelService(this, "/Item/UploadCancle", { temp: temp }, onSuccess);
    }
}
